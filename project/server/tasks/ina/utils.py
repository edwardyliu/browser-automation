...
def next_cache_key(prefix:str, postfix:str)->str:
    """Get the next available file path '<prefix>r"([0-9]+)"<postfix>'
...
def get_absolute_path(filename:str)->str:
    """Get an available absolute file path
    ...
    Returns
    -------
    str: An available absolute file path 
    """

    return os.path.join(config.CACHE_DIRPATH, filename)
